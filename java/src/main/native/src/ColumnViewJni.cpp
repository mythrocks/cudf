...
JNIEXPORT jlongArray JNICALL Java_ai_rapids_cudf_ColumnView_stringContainsMulti(JNIEnv *env, jobject j_object,
                                                                                jlong j_view_handle,
                                                                                jlongArray j_comp_strings) {
  ...
  try {
    ...
    auto comp_string_pointers = cudf::jni::native_jpointerArray<cudf::string_scalar>{env, j_comp_strings};
    auto comp_strings = std::vector<cudf::string_scalar>{};
    std::transform(comp_string_pointers.begin(), comp_string_pointers.end(), std::back_inserter(comp_strings), [](auto i) {return *i;});
//    auto comp_strings_iter = thrust::make_transform_iterator(comp_string_pointers.begin(), [](auto i) -> cudf::string_scalar const& { return *i; });
//    auto comp_strings = std::vector<std::reference_wrapper<cudf::string_scalar>>(comp_strings_iter,
//                                                                                 comp_strings_iter + comp_string_pointers.size());
