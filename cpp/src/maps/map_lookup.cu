...
namespace cudf
{
    ...
    __launch_bounds__(block_size) __global__ void gpu_find_first(
        column_device_view input, // list<struct<string,string>>
        mutable_column_device_view output,
        string_scalar_device_view lookup_key,
    ...
    )
    {
        ...
        size_type block_valid_count = cudf::detail::single_lane_block_sum_reduce<block_size,0>(warp_valid_count);
    ...
    )
    {
        ...
        CUDF_EXPECTS(structs_column.child(0).type().id() == type_id::STRING, "Expected LIST<STRUCT<key,value>>.");
        CUDF_EXPECTS(structs_column.child(1).type().id() == type_id::STRING, "Expected LIST<STRUCT<key,value>>.");

        // Two-pass plan: construct gather map, and then gather() on structs_column.child(1). Plan A.
        // Can do in one pass, but that's Plan B.
