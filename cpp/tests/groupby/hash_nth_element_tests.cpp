...
TEST_F(HashGroupByNthElementTest, Min)
{
  ...
  auto const agg_values    = ints{0, 1, 2, 3, 4, 5, 6, 7};
  ...
  auto agg_request = cudf::groupby::aggregation_request{
    .values = agg_values,
  ...
  auto [grouped, results] = cudf::groupby::groupby(table_view{{grouping_keys}}).aggregate(agg_requests);
