...
struct GroupedRollingRangeOrderByNumericTest : public BaseGroupedRollingRangeOrderByTest<T> {
  ...
  {
    auto const vec = std::vector<T>{-inf, -inf, -50, 0, 7, nan, -inf, 0, inf, nan, 0, inf, nan, nan};
  ...
  void run_test_bounded_ascending_order_by_NaNs_infinity()
  {
    ...
    auto const expected_results = bigints_column{{2, 2, 3, 3, 3, 1, 2, 2, 2, 2, 3, 3, 6, 6},
                                                 cudf::test::iterators::no_nulls()};
    CUDF_TEST_EXPECT_COLUMNS_EQUAL(*results, expected_results);
  }
  ...
  {
    auto const vec = std::vector<T>{nan, 7, 0, -50, -inf, -inf, nan, inf, 0, -inf, nan, nan, inf, 0};
    return fwcw<T>(vec.begin(), vec.end()).release();
  }
  ...
  void run_test_bounded_descending_order_by_NaNs_infinity()
  {
    ...
    auto const results =
        get_grouped_range_rolling_sum_result(preceding, following, *order_by, cudf::order::DESCENDING);

    auto const expected_results = bigints_column{{1, 3, 3, 3, 2, 2, 2, 2, 2, 2, 6, 6, 3, 3},
                                                 cudf::test::iterators::no_nulls()};
    CUDF_TEST_EXPECT_COLUMNS_EQUAL(*results, expected_results);
  }
