...
TEST_F(ScatterListsTest, ListsOfStructs)
{
  ...
  auto source_strings = strings_column_wrapper {
      "nine", "nine","nine","nine", "eight","eight","eight"
  };
  ...
  auto target_ints = fixed_width_column_wrapper<T> {
  ...
  auto target_structs = structs_column_wrapper {
  ...
  auto scatter_result = cudf::scatter(
    cudf::table_view({source_lists->view()}), scatter_map, cudf::table_view({target_lists->view()}));


  auto expected_numerics = fixed_width_column_wrapper<T> {
     8,8,8, 1,1, 9,9,9,9, 3,3, 4,4, 5,5 
  };
...
TEST_F(ScatterListsTest, NullListsOfStructs)
{
  ...
  auto source_list_null_mask_begin = make_counting_transform_iterator(
    0,
    [](auto i) {
      return i != 2;
    }
  ...
  auto expected_lists_null_mask_begin = make_counting_transform_iterator(
    0,
    [](auto i) {
      return i != 4;
    }
